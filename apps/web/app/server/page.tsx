...
import { mcpServerLogs, mcpServerStatus } from "@/app/actions";
import { McpControl } from "@/components/server/mcp-control";

export const dynamic = "force-dynamic";

export default async function ServerPage() {
  const [status, logs] = await Promise.all([mcpServerStatus(), mcpServerLogs(300)]);
  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      ...
      <McpControl initial={status} initialLogs={logs} />
    </main>
  );
}
